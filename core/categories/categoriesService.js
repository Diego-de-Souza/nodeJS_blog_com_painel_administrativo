const { Category } = require('../definition'); // Ajuste o caminho conforme necessário
...
exports.deleteCategory = async function (id, callBack) {
    try {
        const categoriaDeletada = await Category.destroy({
            where: {
                id: id
            }
        });
...
exports.updateTitleCategory = async function (title, id, callBack) {
    try {
        const updateTitleCategory = await Category.update({
            title: title,
            slug: slugify(title)
        }, {
            where: {
